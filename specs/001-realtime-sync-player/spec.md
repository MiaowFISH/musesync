...
- ✅ **IV. 音质增强标准**: Web Audio API 实现 10频段 EQ、空间音效（HRTF）、混响效果
...
5. **Given** 用户开启空间音效，**When** 切换到"演唱会模式"，**Then** 听到明显的空间环绕感和混响效果
...
1. **Given** 房间内有 3 个用户，**When** 用户 A 添加 5 首歌到播放列表，**Then** 所有用户界面同步显示相同的列表
2. **Given** 播放列表有 10 首歌，**When** 任意用户点击"下一首"，**Then** 所有设备同步切换到下一首歌曲
3. **Given** 用户 B 拖动列表调整歌曲顺序，**When** 释放拖动，**Then** 新顺序同步到所有设备，当前播放不受影响
4. **Given** 房间创建者设置"仅主持人控制"，**When** 普通成员尝试切歌，**Then** 显示权限提示，操作被拒绝
...
- **FR-001**: 系统必须支持从网易云音乐 API 搜索和获取歌曲信息（歌名、艺术家、专辑封面、音频 URL）
...
- **FR-004**: 系统必须实现 HRTF 空间音频算法，支持虚拟环绕声和混响效果
...
- **FR-009**: 系统必须在播放进度差异 > 100ms 时自动触发补偿算法，将误差收敛到 < 50ms
...
- **NFR-004**: 播放进度同步误差必须收敛到 < 50ms（目标值）
...
- **Room**: 房间实体，包含 roomId（房间码）、hostId（创建者）、members（成员列表）、playlist（播放列表）、currentTrack（当前播放曲目）、syncState（同步状态对象）
...
- **空间音效**: Web 端使用 PannerNode + ConvolverNode，移动端使用原生 Audio Unit (iOS) 或 Oboe (Android)
...
function calculatePlaybackPosition(syncState: SyncState): number {
  ...
  } else if (Math.abs(drift) > 20) { // 误差 20-100ms
    // 软同步：调整播放速率
    audioPlayer.setPlaybackRate(1.0 + drift * 0.01);
  }
...
- **内容安全**: 仅通过网易云音乐官方 API 获取音频，不存储音频文件
...
| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|----------|
| 网易云音乐 API 不稳定或限流 | 高 | 中 | 实现 API 缓存层，添加备用音源（如 QQ 音乐 API） |
...
| HRTF 空间音效计算量大导致卡顿 | 中 | 低 | 使用 AudioWorklet 在独立线程处理，提供"低质量模式" |
